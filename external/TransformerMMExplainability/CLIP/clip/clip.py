...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit=True, tsm=False, joint=False,T=8,dropout=0., emb_dropout=0.,pretrain=True):
    ...
    if not jit:
        ...
        if str(device) == "cpu":
            model.float()
        return model, model.state_dict() #_transform(model.visual.input_resolution)
...
def tokenize(texts: Union[str, List[str]], context_length: int = 77) -> torch.LongTensor:
    ...
    for i, tokens in enumerate(all_tokens):
        if len(tokens) > context_length:
            # print(f"WARNING: Input \"{texts[i]}\" is too long for context length {context_length}")
            result[i, :context_length-1] = torch.tensor(tokens)[:context_length-1]
            # Insert end token
            result[i, context_length-1] = torch.tensor(tokens)[-1]
        else:
